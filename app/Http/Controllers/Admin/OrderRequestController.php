...
class OrderRequestController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            $data = Order::where('status', 1)->where('seller_approval', 1)->with('product', 'user', 'seller')->latest()->get()->unique('order_track_id');
    ...
    {
        if (request()->ajax()) {
            $data = Order::where('status', 0)->where('seller_approval', 1)->with('product', 'user', 'seller')->latest()->get()->unique('order_track_id')->unique('order_track_id');
            return DataTables::of($data)
                ->addIndexColumn()
                ->editColumn('seller_id', function ($data) {
                    $name = $data->seller->name ?? '<span class="btn btn-sm btn-warning rounded-0">Not Updated</span>';
    ...
    {
        if (request()->ajax()) {
            $data = Order::where('status', 2)->where('seller_approval', 1)->with('product', 'user', 'seller')->latest()->get()->unique('order_track_id')->unique('order_track_id');
            return DataTables::of($data)
                ->addIndexColumn()
                ->editColumn('seller_id', function ($data) {
                    $name = $data->seller->name ?? '<span class="btn btn-sm btn-warning rounded-0">Not Updated</span>';
    ...
    {
        if (request()->ajax()) {
            $data = Order::where('status', 3)->where('seller_approval', 1)->with('product', 'user', 'seller')->latest()->get()->unique('order_track_id')->unique('order_track_id');
            return DataTables::of($data)
                ->addIndexColumn()
                ->editColumn('seller_id', function ($data) {
                    $name = $data->seller->name ?? '<span class="btn btn-sm btn-warning rounded-0">Not Updated</span>';
    ...
    {
        if (request()->ajax()) {
            $data = Order::whereIn('status', [0, 1, 3])->where('seller_approval', 4)->with('product', 'user', 'seller')->latest()->get()->unique('order_track_id')->unique('order_track_id');
            return DataTables::of($data)
                ->addIndexColumn()
                ->editColumn('seller_id', function ($data) {
                    $name = $data->seller->name ?? '<span class="btn btn-sm btn-warning rounded-0">Not Updated</span>';
